...
for step in pass1 pass2 pass3; do
    ...
    case $step in
        "pass1")
            ...
            mkdir -p /osmnt
            if ! mountpoint -q "/osmnt"; then
                mount -t ext4 "$srchdinfo$srcptinfo" /osmnt || { echo "Mount failed"; exit 1; }
            fi
            ...
            # Ambil nilai terbesar antara Used dan Available kolom dari df (kolom 3 dan 4)
            read used available <<< $(df -B1 | grep "$srchdinfo$srcptinfo" | awk '{print $3, $4}')
            if [ "$used" -gt "$available" ]; then
                partavaliable=$used
            else
                partavaliable=$available
            fi
